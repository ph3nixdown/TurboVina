...
while p == "r":
	...
	#option to skip flex file input
	a = "n"
	a = input(["run with flex? type y for yes any other to skip"])
	if a == "y":
		#to allow to run with flexible receptors
		flex = input(["name of flex file?"])	
		sflex = str (flex)
		print("the flex file is named " + sflex + " !")	
	#receptor file name
...
if a == "y" :
	while lignum1 > 0 :
		...
		subprocess.call(r"C:\Program1\TSRI\vina\vina.exe" + " --receptor " + freceptor + " --ligand " + fligand\
						 ...
						 + " --flex " + sflex\
						 + " --out C:\\Program1\\TSRI\\vina\\output\\" + slignum + "_out.pdbqt")
